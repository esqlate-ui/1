...
def _admin_menu_kb(reports_count: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="💬 Чаты",         callback_data="adm:chats"),
         InlineKeyboardButton(text=f"⚠️ Жалобы ({reports_count})", callback_data="adm:reports")],
        [InlineKeyboardButton(text="💰 Платежи",      callback_data="adm:payments"),
         InlineKeyboardButton(text="📢 Рассылка",     callback_data="adm:broadcast")],
    ])

async def _show_admin_menu(target, edit: bool = False):
...
async def admin_menu(message: Message):
    if not adm(message.from_user.id):
        return
    await _show_admin_menu(message)

@router.callback_query(F.data == "adm:menu")
async def adm_back_menu(callback: CallbackQuery):
    if not adm(callback.from_user.id):
        return
    await _show_admin_menu(callback, edit=True)
...
async def adm_users(callback: CallbackQuery):
    ...
    for u in users[:20]:
        ...
        prem_icon = "👑 " if db.is_premium(u["user_id"]) else ""
        rows.append([InlineKeyboardButton(
            text=f"{ban_icon}{prem_icon}{u['name']}, {u['age']}л | @{u.get('username') or '—'}",
...
async def adm_user_detail(callback: CallbackQuery):
    ...
    prem_status = "👑 Premium" if db.is_premium(user_id) else "Нет"
    ban_until  = ""
    if u.get("ban_until"):
    ...
    if u.get("premium_until"):
        t = time.strftime("%d.%m.%Y", time.localtime(u["premium_until"]))
        prem_until = f" (до {t})"
    ...
    text = (
        ...
        f"Premium: {prem_status}{prem_until}\n"
...
async def adm_profiles(callback: CallbackQuery):
    ...
    for p in profiles[:15]:
        prem = "👑 " if p.get("premium") else ""
        rows.append([InlineKeyboardButton(
            text=f"{prem}{p['name']}, {p['age']}л — {(p['description'] or '')[:25]}...",
...
async def adm_chats(callback: CallbackQuery):
    ...
    for c in chats[:20]:
        ...
        closed = " 🔒" if c.get("closed") else ""
        rows.append([InlineKeyboardButton(
            text=f"#{c['id']} {sn} → {tn} ({c.get('msg_count', 0)} сооб.){closed}",
...
async def adm_chat_detail(callback: CallbackQuery, bot: Bot):
    ...
    for m in messages[-30:]:
        ...
        try:
            if m["msg_type"] == "text":
                await bot.send_message(callback.from_user.id, f"[{ts}] <b>{who}:</b> {m['content']}", parse_mode="HTML")
...
async def adm_payments(callback: CallbackQuery):
    ...
    rows_data = [dict(zip(cols, r)) for r in c.fetchall()]
    conn.close()
